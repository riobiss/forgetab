...
async function canJoinCampaignRoom(payload: JoinRoomPayload, userId: string) {
  const permission = await rpgCampaignAccessService.getPermission(payload.rpgId, userId)
  ...
  const campaign = await prismaRpgCampaignRepository.getCampaignSummary(payload.rpgId, payload.campaignId)
  if (!campaign?.isActive) {
    return false
  }

  return prismaRpgCampaignRepository.hasJoinedCampaign(payload.campaignId, userId)
}
...
export function initializeCampaignSocketServer(server: HttpServer) {
  ...
  io = new Server(server, {
    ...
    cors: {
      origin: (origin, callback) => {
        ...
        callback(allowedOrigin ? null : new Error("Origin nao permitida."), Boolean(allowedOrigin))
      },
      credentials: true,
    },
  })
  ...
  io.on("connection", (socket) => {
    socket.on("campaign:join-room", async (payload: JoinRoomPayload) => {
      ...
      if (!allowed) {
        socket.emit("campaign:error", { message: "Voce nao pode entrar nessa sala." })
        return
      }
