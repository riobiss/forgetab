...
const rawPrismaRpgMembershipRepository: RpgMembershipRepository = {
  async getRpgSummary(rpgId) {
    ...
    const rows = await prisma.$queryRaw<
      Array<{ id: string; ownerId: string; visibility: "private" | "public" }>
    >(Prisma.sql`
      SELECT
        id,
        owner_id AS "ownerId",
  ...
  async getMembership(rpgId, userId) {
    const rows = await prisma.$queryRaw<
      Array<{ id: string; status: "pending" | "accepted" | "rejected" }>
    >(Prisma.sql`
      SELECT id, status
      FROM rpg_members
      WHERE rpg_id = ${rpgId}
  ...
  async processCharacterOffer(rpgId, offerId, userId, nextStatus) {
    const rows = await prisma.$transaction(async (tx) => {
      const offerRows = await tx.$queryRaw<
        Array<{ id: string; characterId: string }>
      >(Prisma.sql`
        SELECT
          o.id,
          o.character_id AS "characterId"
      ...
      const updatedOffers = await tx.$queryRaw<
        Array<{ id: string }>
      >(Prisma.sql`
