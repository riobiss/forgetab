...
import type { CharacterSkillPurchaseService } from "@/features/world/character/application/abilities/ports/CharacterSkillPurchaseService.js"
import { prisma } from "@/lib/prisma"
import {
  parseCharacterAbilities,
  parseCostPoints,
} from "@/features/world/character/infrastructure/abilities/services/characterAbilityCostParser.js"
...
function mapSchemaErrors(error: unknown, fallbackMessage: string): never {
  if (
    ...
    throw new AppError(
      "Estrutura de custos desatualizada. Rode a migration mais recente.",
      500,
    )
  }
...
export const characterSkillPurchaseService: CharacterSkillPurchaseService = {
  async buySkill(characterId, userId, params) {
    try {
      return await prisma.$transaction(async (tx) => {
        ...
        const classSkillRows = await tx.$queryRaw<
    ...
    try {
      return await prisma.$transaction(async (tx) => {
        ...
        const refundPoints = character.costsEnabled
          ? (parseCostPoints(skillLevel.cost) ?? 0)
          : 0
        ...
        const updated = await tx.rpgCharacter.update({
          ...
          data: {
            skillPoints:
              refundPoints > 0 ? { increment: refundPoints } : undefined,
            abilities: nextAbilities as Prisma.InputJsonValue,
            updatedAt: new Date(),
          },
          select: { skillPoints: true },
