...
import type { RpgAccessRepository } from "@/features/world/character/application/ports/RpgAccessRepository.js"
...
export const prismaRpgAccessRepository: RpgAccessRepository = {
  async getRpgAccessRow(rpgId) {
    ...
    try {
      rows = await prisma.$queryRaw<
        NonNullable<
          Awaited<ReturnType<RpgAccessRepository["getRpgAccessRow"]>>
        >[]
      >(Prisma.sql`
        SELECT
          owner_id AS "ownerId",
  ...
  async getMembership(rpgId, userId) {
    const rows = await prisma.$queryRaw<
      NonNullable<Awaited<ReturnType<RpgAccessRepository["getMembership"]>>>[]
    >(Prisma.sql`
