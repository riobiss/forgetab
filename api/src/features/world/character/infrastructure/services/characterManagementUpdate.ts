import type { UpdateCharacterPayload } from "@/features/world/character/application/use-cases/updateCharacter.js"
import {
...
} from "@/features/world/character/application/validators.js"
import { prisma } from "@/lib/prisma"
import { resolveProgressionTierByCurrent } from "@/lib/rpg/progression"
import { Prisma } from "../../../../../../generated/prisma/client.js"
import {
  fail,
  rethrowCharacterManagementInfrastructureError,
...
} from "@/features/world/character/infrastructure/services/characterManagementSupport.js"
...
export async function updateCharacterWithLegacyManagement(params: {
  ...
  try {
    ...
    const hasRaceKeyInBody = Object.prototype.hasOwnProperty.call(
      body,
      "raceKey",
    )
    const hasClassKeyInBody = Object.prototype.hasOwnProperty.call(
      body,
      "classKey",
    )
    const canEditTemplateRaceClass =
      permission.characterType === "player" && permission.isOwner
    const shouldPersistRaceKey =
      permission.characterType === "player" && hasRaceKeyInBody
    const shouldPersistClassKey =
      permission.characterType === "player" && hasClassKeyInBody
    if (
      (shouldPersistRaceKey || shouldPersistClassKey) &&
      !canEditTemplateRaceClass
    ) {
      fail(
        403,
    ...
    const name = hasNameInBody
      ? (body.name?.trim() ?? "")
      : permission.currentName
    ...
    if (
      hasNameInBody &&
      (requiresLongerName ? name.length < 2 : name.length === 0)
    ) {
      fail(
        400,
        requiresLongerName
    ...
    const resolvedMaxCarryWeight =
      permission.useInventoryWeightLimit &&
      permission.characterType === "player"
        ? parsedMaxCarryWeight.value
        : null

    const parsedProgressionCurrent = validateProgressionCurrent(
      body.progressionCurrent,
    )
    ...
    const selectedRaceKey = shouldPersistRaceKey
      ? normalizeOptionalText(body.raceKey)
      : null
    const selectedClassKey = shouldPersistClassKey
      ? normalizeOptionalText(body.classKey)
      : null
    ...
    const hasAttributesInBody = Object.prototype.hasOwnProperty.call(
      body,
      "attributes",
    )
    ...
    const hasStatusesInBody = Object.prototype.hasOwnProperty.call(
      body,
      "statuses",
    )
    ...
    const canEditSkills =
      permission.isOwner || permission.characterType !== "player"
    ...
    const incomingIdentity =
      body.identity === undefined ? currentIdentity : body.identity
    ...
    const currentCharacteristics = resolveTextRecord(
