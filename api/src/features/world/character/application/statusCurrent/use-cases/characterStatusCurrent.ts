...
export async function updateCharacterStatusCurrentUseCase(
  ...
  if (
    typeof params.body.value !== "number" ||
    !Number.isFinite(params.body.value)
  ) {
  ...
  const nextCurrentStatuses = Object.entries(currentRecord).reduce<
  ...
  const updated = await repository.updateCharacterStatus(
    params.rpgId,
    params.characterId,
    {
