...
function mapIdentityItemToSecretKey(key: string): string | null {
  ...
  if (
    normalizedKey === "race-free" ||
    normalizedKey === NPC_MONSTER_IDENTITY_KEYS.raceLabel
  ) {
    return "raceLabel"
  }
  if (
    normalizedKey === "class-free" ||
    normalizedKey === NPC_MONSTER_IDENTITY_KEYS.classLabel
  ) {
    return "classLabel"
  }
  if (
    normalizedKey === NPC_MONSTER_IDENTITY_KEYS.titleNickname ||
    normalizedKey.includes("apelido")
  ) {
...
function mapCharacteristicItemToSecretKey(key: string): string | null {
  if (
    key === NPC_MONSTER_CHARACTERISTIC_KEYS.description ||
    key === "description"
  ) {
    return "description"
  }
      ...
      ? [
          {
            ...
            label: "Classe",
            value:
              params.classTemplateLabelByKey.get(params.classKey) ??
              params.classKey,
