...
export async function createCharacter(
  ...
  try {
    ...
    if (
      ...
      !isValidVisibility(input.payload.visibility)
    ) {
    ...
    const dbStatusTemplate =
      await input.rpgTemplatesRepository.getStatusTemplates(input.rpgId)
    ...
    const dbSkillTemplate =
      await input.rpgTemplatesRepository.getSkillTemplates(input.rpgId)
    const defaultSkills = dbSkillTemplate.reduce<Record<string, number>>(
      (acc, item) => {
        acc[item.key] = 0
        return acc
      },
