...
import type { ItemsEditorGateway } from "@/features/world/item/application/editor/contracts/ItemsEditorGateway"
import type { UpsertItemPayloadDto } from "@/features/world/item/application/editor/types"
import {
  createItemUseCase,
...
} from "@/features/world/item/application/editor/use-cases/itemsEditor"
...
describe("itemsEditor use-cases", () => {
  ...
  it("create/update delegam payload para gateway", async () => {
    const gateway = createGatewayMock()
    ;(gateway.createItem as ReturnType<typeof vi.fn>).mockResolvedValue({
      id: "item-1",
      ...payload,
    })
    ;(gateway.updateItem as ReturnType<typeof vi.fn>).mockResolvedValue({
      id: "item-1",
      ...payload,
    })
    ...
    await updateItemUseCase(
      { gateway },
      { rpgId: "rpg-1", itemId: "item-1", payload },
    )
  ...
  it("upload/delete de imagem delegam para gateway", async () => {
    ...
    expect(gateway.deleteItemImageByUrl).toHaveBeenCalledWith(
