...
export async function loadRpgDashboard(
  ...
  let pendingCharacterRequests =
    [] as RpgDashboardViewModel["pendingCharacterRequests"]
  let pendingCharacterOffers =
    [] as RpgDashboardViewModel["pendingCharacterOffers"]
  let acceptedMembers = [] as RpgDashboardViewModel["acceptedMembers"]
  let acceptedMembersCount = 0

  if (canManageRpg) {
    ;[pendingRequests, acceptedMembers, pendingCharacterRequests] =
      await Promise.all([
      ...
      spectatorCharacters = charactersRows.map((character) => {
        ...
        const statusItems = [
          ...
          ...extraStatusEntries.map(([key, value]) => ({
            key,
            label:
              statusTemplateLabelByKey[key] ?? statusLabelByKey[key] ?? key,
            max: Number(value ?? 0),
            current: Math.max(
              0,
              Math.min(
