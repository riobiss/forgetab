...
export async function updateRpg(
  ...
  try {
    ...
    if (
      resolvedAbilityCategoriesEnabled &&
    ...
    const currentProgressionMode =
      await deps.repository.getCurrentProgressionMode(params.rpgId)
    if (
      progressionMode !== undefined &&
      resolvedProgressionMode !== currentProgressionMode
    ) {
      throw new AppError(
