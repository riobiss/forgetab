...
import { parseCharacterAbilities } from "@/features/world/character/infrastructure/abilities/services/characterAbilityCostParser"
...
function isLegacyCampaignMessagesSchemaError(error: unknown) {
  ...
  return (
    ...
    error.message.includes(
      'type "public"."RpgCampaignMessageKind" does not exist'
    )
  )
}
...
function isLegacyCampaignCombatsSchemaError(error: unknown) {
  ...
  return (
    ...
    error.message.includes(
      'relation "rpg_campaign_combat_participants" does not exist'
    ) ||
    error.message.includes(
      'relation "rpg_campaign_combat_queue_entries" does not exist'
    ) ||
...
export const prismaRpgCampaignRepository: RpgCampaignRepository = {
  ...
  async createCampaignMessage(
    ...
    try {
      ...
      if (kind !== "campaign") {
        throw new Error(
          "Schema de chat da campanha ainda nao suporta mensagens diretas ou de acao."
        )
      }
  ...
  async addCreatureCombatants(params) {
    const creatureRows = await prisma.$queryRaw<
    ...
    const currentCountRows = await prisma.$queryRaw<
      Array<{ count: number }>
    >(Prisma.sql`
  ...
  async createCombatQueue(campaignId, combatId) {
    ...
    const entries = fighters
      ...
      .sort(
        (left, right) =>
          right.roll - left.roll || left.label.localeCompare(right.label)
      )
    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      for (const asset of params.assets) {
        ...
        if (
          nextAbilities.some(
            (ability) =>
              ability.skillId === asset.id && ability.level === asset.level
          )
        ) {
          continue
        }
