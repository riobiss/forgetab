...
function makeRequest(method: "GET" | "PUT", body?: unknown, withAuth = true) {
  return new NextRequest("http://localhost/api/rpg/rpg-1/classes", {
    method,
    headers: {
...
describe("classes route", () => {
  ...
  it("GET retorna 200 com classes", async () => {
    mocks.rpgFindUnique.mockResolvedValue({ id: "rpg-1", ownerId: "u1" })
    mocks.queryRaw.mockResolvedValue([
      { id: "c1", key: "guerreiro", label: "Guerreiro", category: "geral", position: 0, attributeBonuses: {}, skillBonuses: {} },
    ])
  ...
  it("PUT retorna 404 sem permissao", async () => {
    ...
    const response = await PUT(makeRequest("PUT", { classes: [] }), makeContext())
