...
export async function POST(request: NextRequest, context: RouteContext) {
  try {
    ...
    if (!characterContext.isOwner) {
      return NextResponse.json(
        { message: "Somente o owner pode dar itens." },
        { status: 403 },
      )
    }

    if (characterContext.character.characterType !== "player") {
      return NextResponse.json(
        { message: "Itens so podem ser dados para personagens player." },
        { status: 400 },
      )
    }
    ...
    return NextResponse.json({ inventory: inventoryItems }, { status: 201 })
  } catch (error) {
    if (
      error instanceof Error &&
      error.message.includes('relation "rpg_character_inventory_items" does not exist')
    ...
    return NextResponse.json(
      { message: "Erro interno ao dar item para o personagem." },
      { status: 500 },
    )
  }
}
