...
function makeRequest(method: "GET" | "PUT", body?: unknown, withAuth = true) {
  return new NextRequest("http://localhost/api/rpg/rpg-1/character-characteristics", {
    method,
    headers: {
...
describe("character-characteristics route", () => {
  beforeEach(() => {
    vi.clearAllMocks()
  ...
  it("GET retorna 200 com fields", async () => {
    ...
    mocks.queryRaw.mockResolvedValue([{ id: "f1", key: "idade", label: "Idade", required: false, position: 0 }])
  ...
  it("PUT retorna 200 ao atualizar", async () => {
    ...
    const response = await PUT(
      makeRequest("PUT", { fields: [{ label: "Idade", required: false }] }),
      makeContext(),
    )
    expect(response.status).toBe(200)
  })
