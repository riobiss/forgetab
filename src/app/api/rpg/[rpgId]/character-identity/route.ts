...
async function canReadCharacterIdentityTemplates(rpgId: string, userId: string) {
...
function normalizeTemplates(input: unknown) {
  ...
  for (const entry of entries) {
    if (!entry || typeof entry !== "object" || Array.isArray(entry)) {
      return { ok: false as const, message: "Campo de identidade invalido." }
    }

    const candidate = entry as IncomingIdentityTemplate
    ...
    const base = slugify(label) || "campo-identidade"
    let key = base
    let suffix = 2
...
export async function GET(request: NextRequest, context: RouteContext) {
  try {
    ...
    const hasAccess = await canReadCharacterIdentityTemplates(rpgId, userId)
    if (!hasAccess) {
      return NextResponse.json({ message: "RPG nao encontrado." }, { status: 404 })
    }

    const fields = await prisma.$queryRaw<IdentityTemplateRow[]>(Prisma.sql`
    ...
    return NextResponse.json(
      { message: "Erro interno ao buscar campos de identidade." },
      { status: 500 },
    )
  }
}
...
export async function PUT(request: NextRequest, context: RouteContext) {
  try {
    ...
    return NextResponse.json({ message: "Campos de identidade atualizados." }, { status: 200 })
  } catch (error) {
    if (
      error instanceof Error &&
      error.message.includes('relation "rpg_character_identity_templates" does not exist')
    ...
    return NextResponse.json(
      { message: "Erro interno ao salvar campos de identidade." },
      { status: 500 },
    )
  }
}
