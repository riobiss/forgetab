...
export async function POST(request: NextRequest) {
  try {
    ...
    if (file.size > MAX_FILE_SIZE_BYTES) {
      return NextResponse.json({ message: "Imagem muito grande. Limite de 8MB." }, { status: 400 })
    }
    ...
    return NextResponse.json({ message: "Erro interno ao enviar imagem." }, { status: 500 })
  }
}
