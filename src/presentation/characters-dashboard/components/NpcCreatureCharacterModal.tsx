...
import { npcCreatureSteps } from "./npc-creature-modal/constants"
import {
  NpcCreatureAbilitiesStep,
  NpcCreatureBasicStep,
  NpcCreatureBonusStep,
  NpcCreatureExtraFieldModal,
  NpcCreatureInventoryStep,
  NpcCreaturePickerModal,
} from "./npc-creature-modal/components"
import {
  ExtraField,
  NarrativeStatus,
  NumericInputValue,
  ...
  SecretFieldKey,
  StepKey,
} from "./npc-creature-modal/types"
import {
  applyCharacterSnapshot,
  createEmptyExtraField,
  parseNumericInputValue,
} from "./npc-creature-modal/utils"
import {
  buildNpcCreatureFormState,
  buildNpcCreatureSecretFieldOptions,
  getNpcCreatureImageStatusText,
} from "./npc-creature-modal/presentation"
...
export default function NpcCreatureCharacterModal({
  rpgId,
  isOpen,
  mode,
  characterType,
  ...
  const [availableItems, setAvailableItems] = useState<NpcCreatureLoadoutItemOptionDto[]>([])
  ...
  const [availableSkills, setAvailableSkills] = useState<NpcCreatureLoadoutSkillOptionDto[]>([])
    ...
    async function loadBootstrap() {
      try {
        ...
        const requests = await Promise.allSettled([
          listNpcCreatureItemOptionsUseCase(loadoutDeps, { rpgId }),
          listNpcCreatureSkillOptionsUseCase(loadoutDeps, { rpgId }),
          target?.id
            ? loadNpcCreatureInventoryUseCase(loadoutDeps, { rpgId, characterId: target.id })
            : Promise.resolve(null),
          target?.id
            ? loadNpcCreatureAbilitiesUseCase(loadoutDeps, { rpgId, characterId: target.id })
            : Promise.resolve(null),
        ])
  ...
  const imageStatusText = getNpcCreatureImageStatusText(image, selectedImageName)
  ...
  const secretFieldOptions = buildNpcCreatureSecretFieldOptions(extraFields)
  const formState = buildNpcCreatureFormState({
    name,
    titleNickname,
    description,
    visibility,
  ...
  async function handleAddInventoryItem(baseItemId: string) {
    ...
    try {
      ...
      setInventoryError("")
      await addNpcCreatureInventoryItemUseCase(loadoutDeps, {
        rpgId,
        characterId: createdCharacterId,
        baseItemId,
        quantity: 1,
      })
      const payload = await loadNpcCreatureInventoryUseCase(loadoutDeps, {
        rpgId,
        characterId: createdCharacterId,
      })
  ...
  async function handleRemoveInventoryItem(inventoryItemId: string, quantity = 1) {
    ...
    try {
      setInventoryError("")
      const payload = await removeNpcCreatureInventoryItemUseCase(loadoutDeps, {
        rpgId,
        characterId: createdCharacterId,
        inventoryItemId,
        quantity,
  ...
  async function handleAddAbility(skillId: string) {
    ...
    try {
      setPickerSaving(true)
      setAbilitiesError("")
      const payload = await addNpcCreatureAbilityUseCase(loadoutDeps, {
        rpgId,
        characterId: createdCharacterId,
        skillId,
        level: 1,
      ...
      if (payload.ability) {
        ...
        const refreshed = await loadNpcCreatureAbilitiesUseCase(loadoutDeps, {
          rpgId,
          characterId: createdCharacterId,
        })
  ...
  async function handleRemoveAbility(skillId: string, level: number) {
    ...
    try {
      ...
      setAbilitiesError("")
      const payload = await removeNpcCreatureAbilityUseCase(loadoutDeps, {
        rpgId,
        characterId: createdCharacterId,
        skillId,
        level,
