...
import { updateRpgProfileClientUseCase } from "@/application/profile/use-cases/updateProfileClient"
...
export default function ProfileRpgNicknameField({ rpgId, nickname }: Props) {
  ...
  async function save() {
    ...
    try {
      ...
      setError(error instanceof Error ? error.message : "Erro ao atualizar apelido.")
    } finally {
      setIsSaving(false)
    }
  }
  ...
  return (
    <strong className={styles.editableValue}>
      ...
      <button
        ...
        aria-label={nickname ? "Editar apelido no RPG" : "Adicionar apelido no RPG"}
