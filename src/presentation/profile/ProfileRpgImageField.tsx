...
} from "@/application/profile/use-cases/updateProfileClient"
...
async function createRoundCroppedFile(imageSrc: string, crop: Area): Promise<File> {
...
export default function ProfileRpgImageField({ rpgId, imageUrl, fallbackName }: Props) {
  ...
  async function saveImage() {
    ...
    try {
      ...
      setError(error instanceof Error ? error.message : "Erro ao salvar imagem.")
    } finally {
      setIsSaving(false)
    }
  }
    ...
    <div className={styles.profileImageField}>
      <div
        ...
        style={
          imageUrl
            ? undefined
            : { backgroundColor: getFallbackBackground(`${rpgId}:${fallbackName}`) }
        }
      >
        {imageUrl ? (
      ...
      {error && !sourceImage ? <small className={styles.editableError}>{error}</small> : null}

      {sourceImage ? (
        <div className={styles.cropBackdrop} role="dialog" aria-modal="true" aria-label="Imagem">
