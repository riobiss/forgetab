...
export function CampaignChatPanel({
  ...
  const [selectedUserId, setSelectedUserId] = useState<string | null>(room.participants[0]?.userId ?? null)
  ...
  useEffect(() => {
    if (
      selectedUserId &&
      availableDirectParticipants.some((participant) => participant.userId === selectedUserId)
    ) {
      return
    }
  ...
  function renderChatMessage(message: CampaignRoomMessage, showAuthor: boolean) {
