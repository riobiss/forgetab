...
export function ItemActionDetailCard({ item, titleAsButton = false, onTitleClick }: ItemCardProps) {
      ...
      {cardItem.coreStats && cardItem.coreStats.length > 0 ? (
        <div className={abilityStyles.cardDetailsGrid}>
          {cardItem.coreStats.map((stat, index) => (
            <div key={`${cardItem.id}-stat-${index}`} className={abilityStyles.detailItem}>
              <span className={abilityStyles.detailLabelOrange}>{stat.label}</span>
    ...
    <div className={styles.itemHighlightBlock}>
      ...
      <div className={styles.itemHighlightList}>
        {entries.map((entry, index) => (
          <div key={`${cardItem.id}-${key}-${index}`} className={styles.itemHighlightItem}>
