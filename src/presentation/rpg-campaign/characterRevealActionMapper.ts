...
export const CHARACTER_REVEAL_FIELD_LABEL: Record<CharacterRevealField, string> = {
  statuses: "Status",
...
export function createInitialCharacterRevealFields(): Record<CharacterRevealField, boolean> {
  return {
    statuses: false,
    attributes: false,
    skills: false,
...
export function getAvailableRevealFields(character: CharacterDetailViewModel): CharacterRevealField[] {
  const fields: CharacterRevealField[] = []
  if (character.statusEntries.length > 0 && !character.maskStatuses) fields.push("statuses")
  if (character.attributeEntries.length > 0 && !character.maskAttributes) fields.push("attributes")
  if (character.skillEntries.length > 0 && !character.maskSkills) fields.push("skills")
...
export function buildCharacterRevealSections(
  ...
  if (fields.progression) {
    sections.push({
      ...
      title: "Progressao",
      entries: [
        { key: "level", label: "Level", value: character.progressionLevelDisplay },
