...
export function OwnerCharacterPickerModal({
    ...
    ? characters.filter((character) => {
        if (mode === "player") {
          return (
            ...
            Boolean(character.createdByUserId && participantUserIds.has(character.createdByUserId))
          )
        }
  ...
  function getCharacterOwnerName(character: DashboardCharacterSummary) {
    ...
    return participants.find((participant) => participant.userId === character.createdByUserId)?.name ?? null
  }
      ...
      >
        ...
        <div className={modalStyles.listBlock}>
          ...
          {isLoading ? (
            ...
            <p className={styles.emptyState}>Escolha uma categoria para ver as fichas disponiveis.</p>
          ) : visibleCharacters.length === 0 ? (
            <p className={styles.emptyState}>Nenhum personagem encontrado nessa categoria.</p>
          ) : (
