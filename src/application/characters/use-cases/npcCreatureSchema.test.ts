...
} from "./npcCreatureSchema"

describe("npcCreatureSchema", () => {
  it("le o draft basico de npc a partir do snapshot do personagem", () => {
    const draft = readNpcCreatureBasicDraft({
      id: "char-1",
      name: "Goblin",
      image: null,
      characterType: "npc",
  ...
  it("monta o payload basico com schema explicita de npc", () => {
    const payload = buildNpcCreatureBasicUpdatePayload({
      currentCharacter: {
        id: "char-1",
        name: "Goblin",
        image: null,
  ...
  it("persiste e le os campos secretos do status secreto", () => {
    ...
    expect(getNpcCreatureSecretFieldKeys(payload.characteristics)).toEqual([
      "name",
      "classLabel",
      "extra:origem",
    ])
