import { formatRollFormula } from "@/features/dices/application/diceRollPresentation"
...
function isSingleDiceRoll(roll: RollHistoryItem) {
  return (
    ...
    roll.groups[0]?.diceCount === 1 &&
    roll.groups[0]?.results.length === 1
  )
}
    ...
    <div className={styles.rollSummary}>
      ...
      <div className={styles.rollMetrics}>
        ...
        {shouldShowRecentRolls ? (
          <div
            className={styles.recentRollsPanel}
            aria-label="Ultimas rolagens"
          >
            <span>Rolagens</span>
            <div>
              {recentSingleRollResults.map((result, index) => (
                <strong
                  ...
                  className={
                    index >= 2 ? styles.recentRollHiddenOnMobile : undefined
                  }
                >
                  {result}
                </strong>
