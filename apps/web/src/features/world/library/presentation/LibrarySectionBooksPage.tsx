...
export default function LibrarySectionBooksPage({
  ...
  function openEditModal(book: LibraryBookDto) {
    ...
    setEditAllowedUserIds(
      Array.isArray(book.allowedCharacterIds) ? book.allowedCharacterIds : [],
    )
    setEditAllowedRaceKeys(
      Array.isArray(book.allowedRaceKeys) ? book.allowedRaceKeys : [],
    )
    setEditAllowedClassKeys(
      Array.isArray(book.allowedClassKeys) ? book.allowedClassKeys : [],
    )
    setEditError("")
  }
          ...
          <div className={styles.grid}>
            {filteredBooks.map((book) => (
              <article
                ...
                onClick={() =>
                  router.push(
                    `/rpg/${rpgId}/library/${sectionId}/books/${book.id}`,
                  )
                }
                onKeyDown={(event) => {
              ...
              <>
                <label className={styles.field}>
                  ...
                  <ReactMultiSelectField
                    ...
                    onChange={(options) =>
                      setEditAllowedUserIds(options.map((item) => item.value))
                    }
          ...
          <section className={`${styles.modal} ${styles.createModal}`}>
            ...
            <div className={styles.modalActions}>
              <button
                type="button"
                className={styles.primaryButton}
                onClick={() => void handleCreateDraftBook()}
              >
                <Plus size={16} />
