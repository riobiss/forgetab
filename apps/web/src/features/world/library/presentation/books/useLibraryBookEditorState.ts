...
export function useLibraryBookEditorState({
  ...
  function extractPlainText(node: JSONContent | null | undefined): string {
    ...
    const children = Array.isArray(node.content)
      ? node.content
          .map((child) => extractPlainText(child as JSONContent))
          .join("")
      : ""
    ...
    async function loadBook() {
      ...
      if (!isEdit || !bookId) {
        ...
        const shouldRestoreDraft =
          ...
          serializeLibraryBookDraft(restoredDraft as LibraryBookDraft) !==
            defaultSignature
      ...
      try {
        ...
        const serverDraft = createLibraryBookDraftSnapshot({
          ...
          allowedCharacterIds: Array.isArray(book.allowedCharacterIds)
            ? book.allowedCharacterIds
            : [],
          allowedClassKeys: Array.isArray(book.allowedClassKeys)
            ? book.allowedClassKeys
            : [],
  ...
  async function saveBook() {
    ...
    try {
      ...
      const message =
        cause instanceof Error ? cause.message : "Erro de conexao ao salvar."
      setError(message)
      toast.error(message)
    } finally {
  ...
  const hasDraft =
    ...
    canEdit &&
    !forceReadOnly &&
    serializeLibraryBookDraft(createLibraryBookDraftSnapshot(draft)) !==
      savedSignature
