...
import type { LibraryAccessService } from "@/features/world/library/application/ports/LibraryAccessService.js"
import type {
  LibraryRepository,
  ViewerCharacter,
...
function ensureCanManageSection(
  ...
  if (owner.createdByUserId !== userId) {
    throw new AppError(
      "Voce so pode editar ou remover secoes criadas por voce.",
      403,
    )
  }
}
...
function ensureCanManageBook(
  ...
  if (owner.createdByUserId !== userId) {
    throw new AppError(
      "Voce so pode editar ou remover livros criados por voce.",
      403,
    )
  }
}
  ...
  try {
    ...
    return {
      sections: (access.canManage
        ? sections
        : sections.filter((section) => section.visibility === "public")
      ).map((section) => ({
        ...section,
