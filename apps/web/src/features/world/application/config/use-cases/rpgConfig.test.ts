...
describe("rpgConfig use-cases", () => {
  ...
  it("updateRaceTemplates usa allowed keys e normaliza lore", async () => {
    ...
    expect(
      (repository.replaceRaceTemplates as ReturnType<typeof vi.fn>).mock
        .calls[0]?.[1]?.[0],
    ).toMatchObject({
      key: "elfo",
      label: "Elfo",
  ...
  it("getClassTemplates converte category null para geral", async () => {
    ...
    const repository = createRepositoryMock()
    ;(
      repository.listClassTemplates as ReturnType<typeof vi.fn>
    ).mockResolvedValue([
      {
        id: "c1",
        key: "guerreiro",
  ...
  it("updateCharacteristicTemplates gera chaves unicas", async () => {
    ...
    expect(repository.replaceCharacteristicTemplates).toHaveBeenCalledWith(
      "rpg-1",
      [
