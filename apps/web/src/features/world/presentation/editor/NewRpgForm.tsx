...
export default function NewRpgForm({
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const message =
        cause instanceof Error ? cause.message : "Erro de conexao ao criar RPG."
      setError(message)
      toast.error(message)
    } finally {
    ...
    <main className={presentation === "embedded" ? undefined : styles.page}>
      <section className={styles.card}>
        ...
        <form className={styles.form} onSubmit={handleSubmit}>
          ...
          {selectedImageFile ? (
            <p>Imagem selecionada: {selectedImageFile.name}</p>
          ) : null}
          ...
          >
            ...
            >
              ...
              <div className={styles.modalOptions}>
                {(["xp_level", "rank", "custom"] as ProgressionMode[]).map(
                  (mode) => (
                    <button
                      key={mode}
                      type="button"
