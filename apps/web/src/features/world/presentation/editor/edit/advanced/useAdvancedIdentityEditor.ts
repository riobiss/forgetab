...
export function useAdvancedIdentityEditor(params: {
  ...
  const mode: "create" | "edit" =
    params.templateKey === "new" ? "create" : "edit"
      ...
      try {
        ...
        if (mode === "create") {
          setDraft({
            ...
            label: "",
            category: "geral",
            attributeBonuses: attributeKeys.reduce<Record<string, number>>(
              (acc, key) => {
                acc[key] = 0
                return acc
              },
  ...
  async function handleSave() {
    ...
    const parsedAttributes = attributeKeys.reduce<Record<string, number>>(
      (acc, key) => {
    ...
    const parsedSkills = skillTemplates.reduce<Record<string, number>>(
      (acc, item) => {
    ...
    try {
      ...
      const message =
        cause instanceof Error ? cause.message : "Nao foi possivel salvar."
      setError(message)
      toast.error(message)
    } finally {
      setSaving(false)
