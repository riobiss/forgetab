...
import type { RpgDashboardDependencies } from "@/features/world/presentation/dashboard/dependencies"
...
function createDeps(): RpgDashboardDependencies {
  return {
    gateway: {
      requestToJoinRpg: vi
        .fn()
        .mockResolvedValue({ message: "Solicitacao enviada." }),
      processMemberRequest: vi
        .fn()
        .mockResolvedValue({ message: "Solicitacao aprovada." }),
      processCharacterRequest: vi
        .fn()
        .mockResolvedValue({ message: "Solicitacao aprovada." }),
      expelMember: vi.fn(),
...
describe("useMembershipNotifications", () => {
  ...
  it("processRequest define erro quando gateway falha", async () => {
    const deps = createDeps()
    ;(
      deps.gateway.processMemberRequest as ReturnType<typeof vi.fn>
