...
import { useQuickCreateMenu } from "@/features/world/presentation/dashboard/hooks/useQuickCreateMenu"
    ...
    <div ref={wrapperRef} className={styles.quickCreateWrapper}>
      ...
      {isOpen ? (
        <div className={styles.quickCreateMenu}>
          ...
          <Link
            href={`/rpg/${rpgId}?modal=edit&editor=rpg`}
            onClick={() => setIsOpen(false)}
          >
            <Users size={15} />
            Configurar RPG
          ...
          >
            <Backpack size={15} />
            {showPointsPanel
              ? "Ocultar distribuicao de pontos"
              : "Distribuir pontos de classe"}
          </button>

          {showPointsPanel ? (
            <div className={styles.quickCreatePointsPanel}>
              <Link
                href={`/rpg/${rpgId}/items`}
                onClick={() => setIsOpen(false)}
              >
                <Backpack size={15} />
                Itens
                  ...
                  <div key={player.id} className={styles.quickCreatePlayerRow}>
                    ...
                    <div className={styles.quickCreateGrantActions}>
                      <button
                        ...
                        onClick={() =>
                          void handleGrantPoint(player.id, player.name, -1)
                        }
              ...
              {panelError ? (
                <p className={styles.quickCreateError}>{panelError}</p>
              ) : null}
              {panelMessage ? (
                <p className={styles.quickCreateSuccess}>{panelMessage}</p>
              ) : null}
            </div>
          ) : null}
        </div>
