...
import { useMembersList } from "@/features/world/presentation/dashboard/hooks/useMembersList"
                    ...
                    <button
                      ...
                      className={`${styles.moderatorToggleButton} ${
                        member.role === "moderator"
                          ? styles.moderatorToggleButtonActive
                          : ""
                      }`}
                      onClick={() => toggleModerator(member.id)}
                      disabled={
                        togglingModeratorId === member.id ||
                        expellingId === member.id
                      }
                    >
                      {togglingModeratorId === member.id
                        ? "Salvando..."
            ...
            <div className={styles.memberActionPanel}>
              ...
              {!actionLoading
                ? members.map((member) => {
                    ...
                    return (
                      <div
                        key={`${member.id}:${actionMode}`}
                        className={styles.memberActionPlayerRow}
                      >
                        <div>
                          <strong>@{member.userUsername}</strong>
                          <small>
                            {player
                              ? `Classe: ${player.classLabel}`
                              : "Sem personagem player vinculado"}
                          </small>
                        </div>
                        {actionMode === "points" ? (
                          <div className={styles.memberActionButtons}>
                            <button
                              ...
                              disabled={
                                loadingAny || !selectedAmount || !player
                              }
              ...
              {actionError ? (
                <p className={styles.memberActionError}>{actionError}</p>
              ) : null}
              {actionMessage ? (
                <p className={styles.memberActionSuccess}>{actionMessage}</p>
              ) : null}
            </div>
          ) : null}
        </div>
