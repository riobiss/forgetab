...
export default function NpcMonsterAbilitiesStep({
  ...
  return (
    <div className={styles.modalBody}>
      ...
      {abilitiesLoading ? (
        <p className={styles.modalInfo}>Carregando habilidades...</p>
      ) : null}
      {abilitiesError ? (
        <p className={styles.modalError}>{abilitiesError}</p>
      ) : null}
      {!abilitiesLoading ? (
        abilities.length > 0 ? (
          <div className={styles.modalList}>
            {abilities.map((ability) => (
              <div
                ...
                className={styles.modalListCard}
              >
                <div>
                ...
                <button
                  ...
                  onClick={() =>
                    onRemoveAbility(ability.skillId, ability.levelNumber)
                  }
                  disabled={
                    !characterId ||
                    removingAbilityKey ===
                      `${ability.skillId}:${ability.levelNumber}`
                  }
                >
                  {removingAbilityKey ===
                  `${ability.skillId}:${ability.levelNumber}`
          ...
          <p className={styles.modalInfo}>
            Nenhuma habilidade neste personagem.
          </p>
        )
      ) : null}
