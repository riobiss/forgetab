...
export default function AbilitiesFiltersClient({
  ...
  const [abilityItems, setAbilityItems] =
    useState<PurchasedAbilityViewDto[]>(abilities)
  ...
  const [selectedAbilityModal, setSelectedAbilityModal] =
  ...
  const baseFiltered = useMemo(() => {
    ...
    return abilityItems.filter((ability) => {
      ...
      if (
        ...
        !selectedTypeFilters.includes(ability.skillType ?? "")
      ) {
        return false
      }
      if (
      ...
      if (
        ...
        !selectedTags.some((tag) => ability.skillTags.includes(tag))
      )
        return false
        ...
        <div className={styles.cardGrid}>
          {filtered.map((ability) => (
            <article
              ...
              style={(() => {
                const meta = ability.skillTags[0]
                  ? getSkillTagMeta(ability.skillTags[0])
                  : null
            ...
            >
              ...
              {(() => {
                ...
                const levelDescriptionNormalized =
                  normalizeText(levelDescription)
          ...
          >
            <div className={styles.modalHeader}>
              <h3 className={styles.modalTitle}>
                {selectedAbilityModal.levelName ??
                  selectedAbilityModal.skillName}
              </h3>
              <button
                type="button"
                className={styles.drawerClose}
            ...
            <div className={styles.modalSection}>
              ...
              <p className={styles.modalText}>
                {toCategoryLabel(selectedAbilityModal.skillCategory) ??
                  "Nao informado"}
              </p>
            </div>
            ...
            <div className={styles.modalSection}>
              ...
              <p className={styles.modalText}>
                ...
                {" | "}
                {toActionTypeLabel(selectedAbilityModal.skillActionType) ??
                  "Nao informado"}
              </p>
            </div>
            ...
            <div className={styles.modalSection}>
              ...
              <p className={styles.modalText}>
                Level requerido: {selectedAbilityModal.levelRequired}
              </p>
              {hasText(selectedAbilityModal.prerequisite) ? (
                <p className={styles.modalText}>
                  Pre-requisito: {selectedAbilityModal.prerequisite}
                </p>
              ) : null}
              {selectedAbilityModal.allowedClasses.length > 0 ? (
                <p className={styles.modalText}>
                  Classes permitidas:{" "}
                  {selectedAbilityModal.allowedClasses.join(" | ")}
                </p>
              ) : null}
              {selectedAbilityModal.allowedRaces.length > 0 ? (
                <p className={styles.modalText}>
                  Racas permitidas:{" "}
                  {selectedAbilityModal.allowedRaces.join(" | ")}
                </p>
              ) : null}
            </div>
            ...
            <div className={styles.modalSection}>
              ...
              {selectedAbilityModal.pointsCost !== null ? (
                <p className={styles.modalText}>
                  {selectedAbilityModal.pointsCost}
                </p>
              ) : (
              ...
              {hasText(selectedAbilityModal.costCustom) ? (
                <p className={styles.modalText}>
                  Custo extra: {selectedAbilityModal.costCustom}
                </p>
              ) : null}
            </div>
            ...
            {canManage ? (
              <>
                {removeAbilityError ? (
                  <p className={styles.errorText}>{removeAbilityError}</p>
                ) : null}
