...
import type { CharacterAbilitiesRepository } from "@/features/world/characters/application/abilities/ports/CharacterAbilitiesRepository"
...
export async function loadCharacterAbilitiesUseCase(
  ...
  if (params.userId && !isOwner) {
    ...
    isModerator =
      membership?.status === "accepted" && membership.role === "moderator"
  }
  ...
  const allowedClassMap = classLinkRows.reduce<Record<string, string[]>>(
    (acc, row) => {
