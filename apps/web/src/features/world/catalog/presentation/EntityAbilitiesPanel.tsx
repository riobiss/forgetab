      ...
      <div className={styles.abilityGrid}>
        {skills.map((skill) => {
          const selectedLevelNumber =
            selectedLevelBySkill[skill.skillId] ?? skill.levels[0]?.levelNumber
          const selectedLevel =
            skill.levels.find(
              (level) => level.levelNumber === selectedLevelNumber,
            ) ?? skill.levels[0]
          ...
          const owned =
            ownedBySkill[skill.skillId]?.has(selectedLevel.levelNumber) ?? false
          ...
          const cantAfford = purchase
            ? purchaseActions.points < pointsCost
            : false
          const buyDisabled =
            Boolean(disabledReason) || owned || loading || cantAfford
                ...
                {skill.levels.length > 1 ? (
                  <div className={styles.levelSelector}>
                    <button
                      ...
                      onClick={() =>
                        setOpenLevelSelectorForSkill((prev) =>
                          prev === skill.skillId ? "" : skill.skillId,
                        )
                      }
                    >
              ...
              <div className={styles.levelRow}>
                <div className={styles.levelInfo}>
                  <div className={styles.abilityStats}>
                    ...
                    {hasText(
                      selectedLevel.levelCategory ?? skill.skillCategory,
                    ) ? (
                      <div className={styles.statItem}>
                    ...
                    {hasText(selectedLevel.levelType ?? skill.skillType) ? (
                      <div className={styles.statItem}>
                        <strong>Tipo</strong>
                        {toTypeLabel(
                          selectedLevel.levelType ?? skill.skillType,
                        )}
                      </div>
                    ) : null}
                    {hasText(
                      selectedLevel.levelActionType ?? skill.skillActionType,
                    ) ? (
                      <div className={styles.statItem}>
                        <strong>Acao</strong>
                        {toActionTypeLabel(
                          selectedLevel.levelActionType ??
                            skill.skillActionType,
                        )}
                      </div>
                    ) : null}
                    {skill.skillTags.length > 0 ? (
                      <div
                        className={`${styles.statItem} ${styles.statItemFull}`}
                      >
                        <strong>Tags</strong>
                        {skill.skillTags
                ...
                {purchase ? (
                  <div className={styles.buyAction}>
                    <button
                      ...
                      onClick={() =>
                        void handleBuy(skill.skillId, selectedLevel.levelNumber)
                      }
                    >
                      {owned
                        ? "Comprado"
