...
export default function EntityCatalogClient({
  ...
  async function handleCreate() {
    ...
    const loadingToastId = toast.loading(
      `Criando ${entityType === "class" ? "classe" : "raca"}...`,
    )
  ...
  async function handleSaveManagedCategory() {
    ...
    const itemDraftMap = new Map(
      manageCategoryItems.map((item) => [item.identity, item]),
    )
    ...
    try {
      const nextCollection = manageCategoryCollection.reduce<
        EntityCatalogTemplateRecord[]
      >((acc, item, index) => {
      ...
      {createOpen ? (
        <div className={styles.modalOverlay} role="dialog" aria-modal="true">
          <section className={styles.modal}>
            ...
            {createError ? (
              <p className={styles.description}>{createError}</p>
            ) : null}
      ...
      {manageCategoryOpen ? (
        ...
        >
          ...
          >
                ...
                <div key={item.identity} className={styles.manageItemRow}>
                  ...
                  <button
                    ...
                    onClick={() =>
                      setManageCategoryItems((current) =>
                        current.filter(
                          (entry) => entry.identity !== item.identity,
                        ),
                      )
                    }
            ...
            {manageCategoryError ? (
              <p className={styles.description}>{manageCategoryError}</p>
            ) : null}
