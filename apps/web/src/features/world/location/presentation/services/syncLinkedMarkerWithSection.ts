...
import { updateRpgMapMarkerGroupUseCase } from "@/features/world/location/application/use-cases/rpgMap"
...
export async function syncLinkedMarkerWithSection(params: Params) {
  ...
  const nextMarkerImage =
    ...
    params.linkedMarker.image
  const nextMarkerColor =
    getOptionalStringValue(customFields[SECTION_LINK_COLOR]) ??
    params.linkedMarker.color

  if (params.linkedMarker.visibility === "private") {
    const raw = window.localStorage.getItem(
      `${MARKER_STORAGE_PREFIX}${params.mapId}`,
    )
    const privateGroups = parsePrivateMarkerGroupsFromStorage(
      raw,
      params.markerColors,
    )
  ...
  const currentGroup = params.detail?.markerGroups.find(
    (group) => group.id === params.linkedMarker.groupId,
  )
  if (!currentGroup) {
    return
  }
