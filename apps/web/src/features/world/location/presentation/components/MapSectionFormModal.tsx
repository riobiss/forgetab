...
export function MapSectionFormModal({
  ...
  return (
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-label="Salvar secao"
    >
      <section
        ref={modalRef}
        className={`${styles.modal} ${styles.sectionModal}`}
        tabIndex={-1}
      >
        ...
        <label className={styles.field}>
          ...
          <input
            ...
            placeholder={
              sectionForm.linkedMarkerId
                ? "Opcional se houver marcador vinculado"
                : ""
            }
          />
        </label>
        ...
        <div className={styles.field}>
          <div className={styles.customFieldsHeader}>
            ...
            >
              <Plus size={14} />
              <span>
                {sectionImageUploading ? "Enviando..." : "Adicionar imagem"}
              </span>
            </button>
          </div>
          {sectionForm.images.length > 0 ? (
            <div className={styles.sectionImageEditorGrid}>
              {sectionForm.images.map((imageUrl, index) => (
                <div
                  key={`${imageUrl}-${index}`}
                  className={styles.sectionImageEditorCard}
                >
        ...
        {sectionFormError ? (
          <p className={styles.error}>{sectionFormError}</p>
        ) : null}
