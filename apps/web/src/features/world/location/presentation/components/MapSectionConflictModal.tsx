...
export function MapSectionConflictModal({
  ...
  return (
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-label="Resolver conflito entre secao e marcador"
    >
      <section
        ref={modalRef}
        className={`${styles.modal} ${styles.sectionConflictModal}`}
        tabIndex={-1}
      >
        <h2>Resolver conflito</h2>
        <p className={styles.feedback}>
          O marcador vinculado tem informacoes diferentes nos campos:{" "}
          {pendingSectionConflict.fields.join(", ")}.
        </p>
        <div className={styles.conflictActions}>
          <button
            type="button"
            className={styles.primaryButton}
            onClick={onKeepMarker}
            disabled={saving}
          >
            Manter marcador
          </button>
