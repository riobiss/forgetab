...
} from "@/features/world/location/presentation/hooks/useMapMarkerGroupStore"
import { useRpgMapFormModalState } from "@/features/world/location/presentation/hooks/useRpgMapFormModalState"
import { useRpgMapPageModalFocus } from "@/features/world/location/presentation/hooks/useRpgMapPageModalFocus"
import { useRpgMapSectionModalState } from "@/features/world/location/presentation/hooks/useRpgMapSectionModalState"
...
import { syncLinkedMarkerWithSection } from "@/features/world/location/presentation/services/syncLinkedMarkerWithSection"
...
export function RpgMapPage({
  ...
  const [privateMarkerOptions, setPrivateMarkerOptions] = useState<
    MarkerLinkOption[]
  >([])
  ...
  const loadDetail = useCallback(
    async (mapId: string, preferredSectionId?: string | null) => {
      try {
        ...
        setError("")
        const payload = await loadRpgMapDetailUseCase(httpRpgMapGateway, {
          rpgId,
          mapId,
        })
        setDetail(payload)
  ...
  useEffect(() => {
    ...
    try {
      const raw = window.localStorage.getItem(
        `${MARKER_STORAGE_PREFIX}${selectedMapId}`,
      )
      const groups = parsePrivateMarkerGroupsFromStorage(
        raw,
  ...
  async function handleSaveMap() {
    try {
      ...
      const map = editingMap
        ? await updateRpgMapUseCase(httpRpgMapGateway, {
            rpgId,
            mapId: editingMap.id,
            payload,
          })
  ...
  async function handleDeleteMap(map: RpgMapDto) {
    if (!window.confirm(`Tem certeza que deseja apagar o mapa "${map.title}"?`))
      return
  ...
  async function handleSaveMarkerSectionLink(
    markerId: string,
    ...
    const linkedMarker =
      markerOptions.find((marker) => marker.id === markerId) ?? null
    if (!linkedMarker) {
      return
    }

    const currentLinkedSections = detail.sections.filter(
      (section) => getLinkedMarkerId(section.customFields) === markerId,
    )
    const sectionsToClear = currentLinkedSections.filter(
    ...
    if (sectionId) {
      const targetSection =
        detail.sections.find((section) => section.id === sectionId) ?? null
      if (targetSection) {
  ...
  async function handleSaveSection() {
    ...
    const linkedMarker =
      markerOptions.find(
        (marker) => marker.id === sectionForm.linkedMarkerId,
      ) ?? null
  ...
  async function handleSectionImageChange(
    ...
    try {
      const payload = await uploadRpgMapSectionImageUseCase(httpRpgMapGateway, {
        file,
      })
  ...
  async function handleRemoveSectionImage(imageUrl: string) {
    ...
    setSectionImageUploading(true)
    try {
      await deleteRpgMapSectionImageByUrlUseCase(httpRpgMapGateway, {
        url: imageUrl,
      })
      setSectionForm((current) => ({
        ...current,
  ...
  async function handleResolveSectionConflict(
  ...
  return (
    <div className={styles.page}>
      ...
      <MapSectionFormModal
        ...
        parentOptions={collectSectionOptions(
          detail?.sections ?? [],
          editingSection?.id,
        )}
        markerOptions={markerOptions.map((marker) => ({
          id: marker.id,
          name: marker.name,
        }))}
