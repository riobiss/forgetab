...
} from "@forgetab/world-contracts/campaign"
...
type ApiCampaignParticipantSummary = Omit<
  RpgCampaignParticipantSummary,
  "joinedAt"
> & {
  joinedAt: string
...
type ApiCampaignMessageSummary = Omit<
  RpgCampaignMessageSummary,
  "createdAt"
> & {
  createdAt: string
...
type ApiCampaignCombatParticipantSummary = Omit<
  RpgCampaignCombatParticipantSummary,
  "joinedAt"
> & {
  joinedAt: string
...
> & {
  campaign: Omit<
    RpgCampaignRoomViewModel["campaign"],
    "startedAt" | "endedAt"
  > & {
    startedAt: string | null
    endedAt: string | null
...
const parseJsonResponse = createApiResponseParser({
  fallbackMessage: "Erro ao carregar campanhas.",
  invalidResponseMessage: "Resposta invalida da API.",
...
function toCampaignParticipant(
  item: ApiCampaignParticipantSummary
): RpgCampaignParticipantSummary {
  return {
    ...item,
...
function toCampaignMessage(
  item: ApiCampaignMessageSummary
): RpgCampaignMessageSummary {
  return {
    ...item,
    createdAt: new Date(item.createdAt)
...
function toCampaignCombatRoom(
  item: ApiCampaignCombatRoomSummary
): RpgCampaignCombatRoomSummary {
  return {
    ...item,
    createdAt: new Date(item.createdAt),
...
export async function fetchRpgCampaignViewModel(
  rpgId: string
...
export async function fetchRpgCampaignRoomViewModel(
  ...
  return {
    ...
    campaign: {
      ...payload.campaign,
      startedAt: payload.campaign.startedAt
        ? new Date(payload.campaign.startedAt)
        : null,
      endedAt: payload.campaign.endedAt
        ? new Date(payload.campaign.endedAt)
        : null
    },
...
async function postWithJson(path: string, body: unknown) {
  ...
  const payload = await parseJsonResponse<{
    message?: string
    chatMessage?: ApiCampaignMessageSummary
  }>(response)
  return {
    ...payload,
    chatMessage: payload.chatMessage
      ? toCampaignMessage(payload.chatMessage)
      : undefined
  }
}
...
export const httpRpgCampaignRepository = {
  ...
  createCombat(rpgId: string, campaignId: string, payload: { name: string }) {
    return apiFetch(`/api/rpg/${rpgId}/campaigns/${campaignId}/combats`, {
      ...
      parseJsonResponse<{ message?: string; combatId?: string }>(response)
    )
  },
  joinCombat(
    rpgId: string,
  ...
  async fetchCombatCreatureOptions(rpgId: string) {
    ...
    const payload = await parseJsonResponse<{
      characters?: CampaignCreatureOption[]
    }>(response)
    return (payload.characters ?? []).filter(
      (character) => character.characterType === "monster"
    )
  },
  async fetchCombatItemOptions(rpgId: string) {
    ...
    const payload = await parseJsonResponse<{ items?: CampaignItemOption[] }>(
      response
    )
