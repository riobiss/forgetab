...
export function setCampaignPresencePosition(
  position: CampaignPresencePosition
) {
  if (typeof window === "undefined") return

  window.localStorage.setItem(
    CAMPAIGN_PRESENCE_POSITION_KEY,
    JSON.stringify(position)
  )
}

export function getCampaignSelectedCharacter(
  campaignId: string
): CampaignSelectedCharacter | null {
  if (typeof window === "undefined") return null
