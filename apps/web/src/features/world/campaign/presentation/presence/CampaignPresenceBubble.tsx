...
export default function CampaignPresenceBubble() {
  ...
  function handlePointerDown(event: ReactPointerEvent<HTMLDivElement>) {
    ...
    const currentTop =
      position?.y ??
      Math.max(headerHeight + 16, window.innerHeight - bubbleHeight - 16)
  ...
  function handlePointerMove(event: ReactPointerEvent<HTMLDivElement>) {
    ...
    const nextPosition = clampPosition(
      pointerState.startLeft + deltaX,
      pointerState.startTop + deltaY
    )
  ...
  function handlePointerUp(event: ReactPointerEvent<HTMLDivElement>) {
    ...
    if (!pointerState.moved && activePresence) {
      router.push(
        `/rpg/${activePresence.rpgId}/campaign/${activePresence.campaignId}`
      )
    }
  }
  ...
  return (
    ...
    >
      <button
        ...
        tabIndex={-1}
        aria-label="Abrir campanha atual"
      >
