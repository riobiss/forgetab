...
import type { BaseItemDto } from "@/features/world/items/application/dashboard/types"
...
import type { SkillListItemDto } from "@/features/world/skills/application/skillsDashboard/types"
import { fetchCharacterAbilitiesViewModel } from "@/features/world/characters/infrastructure/abilities/repositories/httpCharacterAbilitiesPageRepository"
...
import type { CampaignSelectedCharacter } from "@/features/world/campaign/infrastructure/presence/campaignPresence"
...
export function useCampaignRoomActions({
  ...
  const [diceEntries, setDiceEntries] = useState<
    Array<{ diceCount: string; diceSides: string }>
  >([{ diceCount: "1", diceSides: "20" }])
  const [dicePreviewGroups, setDicePreviewGroups] = useState<
    DiceRollPreviewGroup[] | null
  >(null)
  ...
  >([])
  const [deliveryCharacters, setDeliveryCharacters] = useState<
  ...
  const [isUploadingLocationImage, setIsUploadingLocationImage] =
    useState(false)
  const [isLoadingCharacterAbilities, setIsLoadingCharacterAbilities] =
    useState(false)
  const [isLoadingCharacterItems, setIsLoadingCharacterItems] = useState(false)
  const [selectedAbilityDetails, setSelectedAbilityDetails] =
    useState<PurchasedAbilityViewDto | null>(null)
  const [selectedAbilityDetailsMode, setSelectedAbilityDetailsMode] = useState<
    "view" | "use"
  >("view")
  const [selectedItemDetails, setSelectedItemDetails] =
    useState<CharacterInventoryItemDto | null>(null)
  const [selectedItemDetailsMode, setSelectedItemDetailsMode] = useState<
    "view" | "use"
  >("view")
  const [expandedActionIds, setExpandedActionIds] = useState<
    Record<string, boolean>
  >({})
  const [expandedRollIds, setExpandedRollIds] = useState<
    Record<string, boolean>
  >({})
  const [revokeActionMessageId, setRevokeActionMessageId] = useState<
    string | null
  >(null)
  ...
  async function handleDiceRoll() {
    ...
    for (const entry of diceEntries) {
      ...
      if (
        !Number.isInteger(nextDiceSides) ||
        nextDiceSides < 2 ||
        nextDiceSides > 1000
      ) {
    ...
    try {
      payload = await httpRpgCampaignRepository.rollDice(
        rpgId,
        room.campaign.id,
        { entries }
      )
  ...
  async function openSkillModal() {
    ...
    try {
      const payload = await fetchCharacterAbilitiesViewModel(
        rpgId,
  ...
  async function openItemModal() {
    ...
    try {
      const payload = await httpCharacterInventoryGateway.fetchInventory(
        rpgId,
        selectedCharacter.id
      )
  ...
  async function openDeliveryModal() {
    ...
    if (
      ...
      deliverySkills.length > 0 ||
      deliveryCharacters.length > 0
    ) {
      return
    }
    ...
    try {
      const [itemsPayload, charactersPayload, skillsPayload] =
        await Promise.all([
      ...
      const skillsIndex =
        await httpSkillsDashboardGateway.fetchSkillsSearchIndex({
      ...
      setError(
        actionError instanceof Error
          ? actionError.message
          : "Nao foi possivel carregar as opcoes de entrega."
      )
    } finally {
  ...
  function buildLocationOptions(
    ...
    const markerOptions: CampaignLocationOption[] = detail.markerGroups.flatMap(
      (group) =>
        group.markers
    ...
    const sectionOptions: CampaignLocationOption[] = detail.sections.map(
      (section) => {
        const linkedMarkerId = getLinkedMarkerId(section.customFields)
        const linkedMarker = linkedMarkerId
          ? (markerById.get(linkedMarkerId) ?? null)
          : null
        const sectionImages = getSectionImages(section.customFields)
        const linkedSectionImage = getCustomFieldText(
          section.customFields?.[SECTION_LINK_IMAGE]
        )
        const resolvedImage =
          sectionImages[0] || linkedSectionImage || linkedMarker?.image || null
        ...
        return {
          ...
          description:
            section.description || linkedMarker?.shortDescription || null,
          image: resolvedImage,
    ...
    try {
      ...
      setLocationOptions(
        details.flatMap((detail) =>
          detail ? buildLocationOptions(detail) : []
        )
      )
    } catch (actionError) {
      setError(
        actionError instanceof Error
          ? actionError.message
          : "Nao foi possivel carregar os locais."
      )
    } finally {
  ...
  async function uploadLocationImage(file: File) {
    ...
    try {
      ...
      setError(
        actionError instanceof Error
          ? actionError.message
          : "Nao foi possivel enviar a imagem do local."
      )
      return null
    } finally {
  ...
  async function submitLocation(
    payload: Omit<LocationActionPayload, "type" | "combatId">
  ) {
    if (!room.isOwner) {
      return
    }
    ...
    await runAction(async () => {
      const response = await httpRpgCampaignRepository.sendMessage(
        rpgId,
        room.campaign.id,
        {
          content,
  ...
  async function submitDeliveryOffer(params: {
    ...
    const content = buildDeliveryOfferActionContent({
      ...
      recipientCharacterIds: params.recipients.map(
  ...
  async function openLatestItemDetails(params: {
    ...
    if (!params.characterId) {
      const cachedItem =
        characterItems.find((item) => item.id === params.item.id) ?? params.item
    ...
    try {
      ...
      setCharacterItems(payload.inventory)
      const latestItem =
        payload.inventory.find((item) => item.id === params.item.id) ??
        params.item
  ...
  async function handleRevokeActionMessage() {
    ...
    await runAction(async () => {
      ...
      setRoom((currentRoom) => ({
        ...currentRoom,
        actionMessages: currentRoom.actionMessages.filter(
  ...
  async function handleAcceptDeliveryOffer(
    ...
    const isTargeted =
      offer.recipientUserIds.length > 0 ||
      offer.recipientCharacterIds.length > 0
    const canReceive =
      !isTargeted ||
    ...
    return runAction(async () => {
      return httpRpgCampaignRepository.acceptDeliveryOffer(
        rpgId,
        room.campaign.id,
        messageId,
        {
  ...
  async function submitDiceRoll(groupsSource: DiceRollPreviewGroup[]) {
    ...
    for (const group of groupsSource) {
      ...
      const hasInvalidValue = results.some(
        (value) =>
          !Number.isInteger(value) || value < 1 || value > group.diceSides
      )

      if (hasInvalidValue) {
        setError(
          `Os resultados de ${group.diceCount}d${group.diceSides} precisam ficar entre 1 e ${group.diceSides}.`
        )
        return
      }
