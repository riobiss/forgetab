...
export function CampaignCreatureCombatModal({
  ...
  const [itemMode, setItemMode] = useState<
    "none" | "same" | "selectedRandom" | "rarityRandom"
  >("none")
      ...
      .catch((error) => {
        ...
        setLoadError(
          error instanceof Error
            ? error.message
            : "Nao foi possivel carregar criaturas."
        )
      })
    ...
    return [
      {
        ...
        label: "Status",
        values: asRecord(selectedCreature.statuses)
      },
      {
        key: "attributes" as const,
        label: "Atributos",
        values: asRecord(selectedCreature.attributes)
      },
      {
        key: "skills" as const,
        label: "Pericias",
        values: asRecord(selectedCreature.skills)
      }
    ]
  }, [selectedCreature])

  function upsertRollConfig(
    target: RollTarget,
    nextDicePerLevel: number,
    nextDiceSides: number
  ) {
    setRollConfigs((currentConfigs) => {
      const withoutTarget = currentConfigs.filter(
        (config) =>
          config.section !== target.section || config.key !== target.key
      )
      return [
        ...withoutTarget,
        { ...target, dicePerLevel: nextDicePerLevel, diceSides: nextDiceSides }
      ]
    })
  }
  ...
  function getRollConfig(section: RollTarget["section"], key: string) {
    return (
      rollConfigs.find(
        (config) => config.section === section && config.key === key
      ) ?? null
    )
  }
  ...
  function buildStatRolls(nextQuantity: number) {
    const level = Math.max(
      1,
      Math.trunc(Number(selectedCreature?.progressionCurrent ?? 1))
    )
    return Array.from({ length: nextQuantity }, (_, creatureIndex) => ({
      ...
      rolls: rollConfigs.map((config) => {
        const diceCount = Math.max(
          1,
          Math.min(100, config.dicePerLevel * level)
        )
  ...
  function buildItemAssignments(
    ...
    const randomPool =
      itemMode === "selectedRandom" ? selectedItems : filteredItems
  ...
  async function submitCreatures() {
    ...
    const nextQuantity = Math.max(
      1,
      Math.min(20, Math.trunc(Number(quantity) || 1))
    )
    const nextItemQuantity = Math.max(
      0,
      Math.min(50, Math.trunc(Number(itemQuantity) || 0))
    )

    await runAction(() =>
      httpRpgCampaignRepository.addCombatCreatures(
        rpgId,
        campaignId,
        combatId,
        {
      ...
      >
        ...
        <div className={styles.creatureCombatModes}>
          <label>
            <input
              type="radio"
              checked={itemMode === "none"}
              onChange={() => setItemMode("none")}
            />{" "}
            Sem itens
          </label>
          <label>
            <input
              type="radio"
              checked={itemMode === "same"}
              onChange={() => setItemMode("same")}
            />{" "}
            Todos recebem os selecionados
          </label>
          <label>
            <input
              type="radio"
              checked={itemMode === "selectedRandom"}
              onChange={() => setItemMode("selectedRandom")}
            />{" "}
            Aleatorio entre selecionados
          </label>
          <label>
            <input
              type="radio"
              checked={itemMode === "rarityRandom"}
              onChange={() => setItemMode("rarityRandom")}
            />{" "}
            Aleatorio por raridade
          </label>
        </div>
              ...
              {Object.entries(section.values).length === 0 ? (
                ...
                Object.entries(section.values).map(([key, value]) => {
                  ...
                  return (
                    <div
                      key={`${section.key}-${key}`}
                      className={styles.creatureRollRow}
                    >
                      <span>{key}</span>
                      ...
                      {config ? (
                        <small>
                          {config.dicePerLevel} por nivel d{config.diceSides}
                        </small>
                      ) : null}
                    </div>
                  )
        ...
        {rollTarget ? (
          ...
          >
            ...
            >
              <h2
                id="creature-roll-title"
                className={styles.confirmActionTitle}
              >
              ...
              <label className={styles.creatureCombatField}>
                ...
                <input
                  ...
                  max={100}
                  value={dicePerLevel}
                  onChange={(event) => setDicePerLevel(event.target.value)}
                />
              </label>
              <label className={styles.creatureCombatField}>
                <span>Lados</span>
                <input
                  ...
                  max={1000}
                  value={diceSides}
                  onChange={(event) => setDiceSides(event.target.value)}
                />
              </label>
              <button
                type="button"
