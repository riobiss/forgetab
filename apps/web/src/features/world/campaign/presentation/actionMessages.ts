...
export type LocationActionPayload = {
  type: "location"
  combatId?: string | null
  title: string
  description?: string | null
...
export function toAbilityDisplayName(
  ability: Pick<PurchasedAbilityViewDto, "levelName" | "skillName">
) {
...
export function buildCharacterRevealActionContent(
  payload: CharacterRevealActionPayload
) {
  return `${CHARACTER_REVEAL_PREFIX}${JSON.stringify(payload)}`
}

export function buildDeliveryOfferActionContent(
  payload: DeliveryOfferActionPayload
) {
...
export function parseDiceRollAction(
  content: string
): DiceRollActionPayload | null {
  ...
  try {
    const parsedContent = JSON.parse(
      content.slice(DICE_ROLL_PREFIX.length)
    ) as DiceRollActionPayload
    if (
...
export function parseSkillUseAction(
  content: string
): SkillUseActionPayload | null {
  ...
  try {
    const parsedContent = JSON.parse(
      content.slice(SKILL_USE_PREFIX.length)
    ) as SkillUseActionPayload
    if (
...
export function parseItemUseAction(
  content: string
): ItemUseActionPayload | null {
  ...
  try {
    const parsedContent = JSON.parse(
      content.slice(ITEM_USE_PREFIX.length)
    ) as ItemUseActionPayload
    if (
...
export function parseCharacterRevealAction(
  content: string
): CharacterRevealActionPayload | null {
  ...
  try {
    const parsedContent = JSON.parse(
      content.slice(CHARACTER_REVEAL_PREFIX.length)
    ) as CharacterRevealActionPayload
    if (
      parsedContent?.type !== "character_reveal" ||
      typeof parsedContent.characterId !== "string" ||
      typeof parsedContent.characterName !== "string" ||
      (parsedContent.characterType !== "npc" &&
        parsedContent.characterType !== "monster") ||
...
export function parseDeliveryOfferAction(
  content: string
): DeliveryOfferActionPayload | null {
...
export function parseLocationAction(
  content: string
): LocationActionPayload | null {
  ...
  try {
    const parsedContent = JSON.parse(
      content.slice(LOCATION_PREFIX.length)
    ) as LocationActionPayload
    if (
      parsedContent?.type !== "location" ||
      typeof parsedContent.title !== "string" ||
      parsedContent.title.trim().length === 0
    ) {
      return null
    }
...
export function parseParchmentAction(
  content: string
): ParchmentActionPayload | null {
  ...
  try {
    const parsedContent = JSON.parse(
      content.slice(PARCHMENT_PREFIX.length)
    ) as ParchmentActionPayload
    if (
