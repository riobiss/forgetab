...
import {
  AbilityActionDetailCard,
  ItemActionDetailCard
} from "./ActionDetailCards"
...
export function CampaignActionMessageCard({
  ...
  const [isCharacterRevealDetailsOpen, setIsCharacterRevealDetailsOpen] =
    useState(false)
  ...
  const [privateChestOffer, setPrivateChestOffer] = useState<NonNullable<
    ReturnType<typeof parseDeliveryOfferAction>
  > | null>(null)
  const [publicChestOffer, setPublicChestOffer] = useState<NonNullable<
    ReturnType<typeof parseDeliveryOfferAction>
  > | null>(null)
  ...
  const canRevokeAction =
    isOwner ||
    (message.authorId === viewerUserId &&
      actionMessages
        .slice(-2)
        .some((actionMessage) => actionMessage.id === message.id))

  if (skillUse) {
    const isStealthHidden =
      Boolean(skillUse.stealth) && !isOwner && !isActionAuthor
    const primaryTag = skillUse.ability.skillTags[0]
    const tagMeta =
      !isStealthHidden && primaryTag ? getSkillTagMeta(primaryTag) : null
    const abilityName = isStealthHidden
      ? "Ação furtiva"
      : toAbilityDisplayName(skillUse.ability)
    const actionTypeLabel = isStealthHidden
      ? "Furtivo"
      : (toActionTypeLabel(skillUse.ability.skillActionType) ?? "Habilidade")
    const canOpenSkillDetails = isStealthHidden
      ? false
      : skillUse.stealth
        ? isOwner || isActionAuthor
  ...
  if (itemUse) {
    const isStealthHidden =
      Boolean(itemUse.stealth) && !isOwner && !isActionAuthor
    const cardItem = toInventoryCardItem(itemUse.item)
    const rarityColor = ITEM_RARITY_ACTION_COLOR[itemUse.item.itemRarity]
    const actionTypeLabel = isStealthHidden
      ? "Furtivo"
      : (cardItem.secondaryLine ?? "Item")
    const canOpenItemDetails = isStealthHidden
      ? false
      : itemUse.stealth
        ? isOwner || isActionAuthor
        : canOpenActionDetails
    ...
    return (
      <article
        ...
        style={
          {
            "--action-summary-text": isStealthHidden
  ...
  if (!diceRoll) {
    ...
    if (deliveryOffer) {
      ...
      const canInteract =
        !isOwner &&
        (!isTargeted || deliveryOffer.recipientUserIds.includes(viewerUserId))
      ...
      const canInspectHiddenChest =
        isOpened && !isRevealed && deliveryOffer.openedByUserId === viewerUserId
      const canInspectChest = canInspectRevealedChest || canInspectHiddenChest

      return (
        <article
          className={`${styles.streamCard} ${isChest ? cardStyles.chestOfferCard : cardStyles.deliveryOfferCard}`}
        >
          ...
          {isChest && isChestChoiceOpen ? (
            <ChestRevealChoiceModal
              ...
              onReveal={async () => {
                ...
                const wasOpened = await onAcceptDeliveryOffer(
                  message.id,
                  deliveryOffer,
                  { revealToRoom: true }
                )
                ...
                if (wasOpened) {
                  ...
                  setPublicChestOffer(
                    markChestOfferOpened(deliveryOffer, viewerUserId, true)
                  )
                }
              }}
              onHide={async () => {
                ...
                if (wasOpened) {
                  setChestAnimationToken((currentToken) => currentToken + 1)
                  setIsChestChoiceOpen(false)
                  setPrivateChestOffer(
                    markChestOfferOpened(deliveryOffer, viewerUserId, false)
                  )
                }
              }}
            />
    ...
    if (characterReveal) {
      return (
        <article
          className={`${styles.streamCard} ${styles.characterRevealStreamCard}`}
        >
          {canRevokeAction ? (
            <button
              type="button"
            ...
            >
              {characterReveal.image ? (
                <span className={styles.characterRevealPortrait}>
                  ...
                  />
                  <strong className={styles.characterRevealName}>
                    {characterReveal.characterName}
                  </strong>
                </span>
              ) : (
  ...
  const isStealthRollHidden =
    Boolean(diceRoll.stealth) && !isOwner && !isActionAuthor
    ...
    <article className={`${styles.streamCard} ${styles.actionStreamCard}`}>
      ...
      <div className={styles.rollGroupList}>
        {diceRoll.groups.map((group, index) => (
          <p
            ...
            className={styles.rollGroupLine}
          >
      ...
      {canOpenRollDetails && isRollExpanded ? (
        <div className={styles.rollSequenceList}>
          {diceRoll.groups.map((group, index) => (
            <p
              key={`${message.id}-sequence-${index}`}
              className={styles.rollSequence}
            >
...
function ParchmentActionCard({
  ...
  return (
    <article className={`${styles.streamCard} ${cardStyles.parchmentCard}`}>
      ...
      <Link
        href={`/rpg/${rpgId}/library/${payload.sectionId}/books/${payload.bookId}`}
        className={cardStyles.parchmentLink}
      >
      ...
      <span className={`${styles.streamTime} ${cardStyles.parchmentTime}`}>
        {time}
      </span>
    </article>
  )
      ...
      if (!player || totalFrames <= 1) {
        if (attempt < 30) {
          retryTimer = window.setTimeout(
            () => startOpeningAnimation(attempt + 1),
            100
          )
        } else {
          isPlayingOpeningRef.current = false
  ...
  const chestPlayer = (
    <span className={cardStyles.chestPlayerShell}>
      {isPlayerReady ? (
        <dotlottie-wc
          ref={playerRef}
          src="/animations/BrahmaChest.json"
          speed={0.45}
  ...
  return (
    <div className={cardStyles.chestScene}>
      {canOpenOffer ? (
        ...
        <div
          className={`${cardStyles.chestButton} ${cardStyles.chestButtonStatic}`}
          aria-label="Bau"
          role="img"
        >
          {chestPlayer}
      ...
      {!isRevealed && isOpened ? (
        <p className={cardStyles.chestSealedHint}>
          O bau foi aberto em segredo.
        </p>
      ) : null}
    </div>
  )
...
function LootPreview({
  ...
  return (
    <div
      className={
        isOpened ? cardStyles.lootGridRevealed : cardStyles.lootGridHidden
      }
    >
      {assets.map((asset) => {
        ...
        return (
          <div
            ...
            className={cardStyles.lootItem}
          >
            <span className={cardStyles.lootName}>{assetName}</span>
            <small>
              {asset.kind === "item"
                ? `Item x${asset.quantity}`
                : `Habilidade Nv.${asset.level}`}
            </small>
          </div>
        )
      })}
...
function formatDeliveryAssetName(
  asset: NonNullable<
    ReturnType<typeof parseDeliveryOfferAction>
  >["assets"][number]
) {
...
function LocationActionCard({
  ...
  useEffect(() => {
    ...
    async function resolveMapImage() {
      try {
        const detail = await httpRpgMapGateway.fetchMap(
          rpgId,
          payload.mapId ?? ""
        )
        const linkedSection = payload.markerId
          ? detail.sections.find(
              (section) =>
                getLinkedMarkerId(section.customFields) === payload.markerId
            )
          : null
        ...
        const linkedSectionImage = getActionCustomFieldText(
          linkedSection?.customFields?.[SECTION_LINK_IMAGE]
        )
        const nextImage =
          sectionImages[0] ||
          linkedSectionImage ||
    ...
    <>
      ...
      <span
        className={`${cardStyles.locationText} ${
          hasDescription
            ? cardStyles.locationTextWithDescription
            : cardStyles.locationTextNameOnly
        }`}
      >
  ...
  return (
    <article className={`${styles.streamCard} ${cardStyles.locationCard}`}>
      ...
      <span className={`${styles.streamTime} ${cardStyles.locationTime}`}>
        {time}
      </span>
    </article>
  )
    ...
    <div className={styles.modalBackdrop} role="presentation" onClick={onClose}>
      ...
      >
        ...
        <p className={cardStyles.chestChoiceText}>
          Voce pode revelar o conteudo para a sala ou guardar a descoberta so
          para voce.
        </p>
        <div className={cardStyles.chestChoiceActions}>
          <button
            type="button"
            className={cardStyles.chestRevealButton}
            onClick={onReveal}
            disabled={isBusy}
          >
            Revelar
          ...
          <button
            type="button"
            className={cardStyles.chestHideButton}
            onClick={onHide}
            disabled={isBusy}
          >
            Esconder
  ...
  return (
    <div className={styles.modalBackdrop} role="presentation" onClick={onClose}>
      ...
      >
        ...
        <LootPreview
          assets={offer.assets}
          isOpened={Boolean(offer.openedAt) || offer.mode !== "chest"}
        />
        <div className={styles.confirmActionButtons}>
          {canAccept ? (
            <button
              type="button"
              className={cardStyles.deliveryAcceptButton}
              onClick={onAccept}
            >
  ...
  return (
    <div className={styles.modalBackdrop} role="presentation" onClick={onClose}>
      ...
      >
        <div className={styles.actionModalHeader}>
          <h2
            id="character-reveal-details-title"
      ...
      <div className={styles.actionCardControls}>
        ...
        {params.canOpenActionDetails ? (
          <button
            ...
            aria-label={
              params.isExpanded
                ? `Recolher ${params.expandLabel}`
                : `Expandir ${params.expandLabel}`
            }
          >
            {params.isExpanded ? (
              <ChevronDown size={16} />
