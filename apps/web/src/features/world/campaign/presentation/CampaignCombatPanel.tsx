...
export function CampaignCombatPanel({
  ...
  const activeRoom =
    rooms.find((combatRoom) => combatRoom.id === activeRoomId) ?? null
    ...
    return (
      <section className={combatStyles.section}>
        <div className={combatStyles.nestedHeader}>
          <button
            type="button"
            className={combatStyles.backButton}
          ...
          <div className={combatStyles.arena}>
            {viewerCombatParticipant ? (
              <p className={combatStyles.roleBanner}>
                Voce entrou como{" "}
                {viewerCombatParticipant.role === "fighter"
                  ? "batalhante"
                  : "espectador"}
                .
              </p>
              ...
              <div className={combatStyles.joinInline}>
                <p className={combatStyles.roleBanner}>
                  Entre como espectador ou batalhante para acompanhar essa sala.
                </p>
                <button
                  type="button"
                    ...
                    <button
                      ...
                      disabled={
                        activeRoom.participants.filter(
                          (participant) => participant.role === "fighter"
                        ).length === 0
                      }
                    >
        ...
        <div className={combatStyles.nestedActions}>
          <div className={combatStyles.nestedActionsHeader}>
            ...
            <small>
              Jogadas e usos feitos enquanto essa sala estiver aberta.
            </small>
          </div>
          {actionContent}
        </div>
        ...
        {queueRoomId ? (
          <CombatQueueModal
            combatRoom={
              rooms.find((combatRoom) => combatRoom.id === queueRoomId) ?? null
            }
            isOwner={isOwner}
        ...
        {rooms.map((combatRoom, index) => (
          ...
          >
            <div className={combatStyles.roomSummary}>
              ...
              <small>
                {
                  combatRoom.participants.filter(
                    (participant) => participant.role === "fighter"
                  ).length
                }{" "}
                batalhando
  ...
  return (
    <div className={styles.modalBackdrop} role="presentation" onClick={onClose}>
      ...
      >
        <button
          type="button"
          className={`${styles.closeChatButton} ${combatStyles.choiceModalCloseButton}`}
          onClick={onClose}
          aria-label="Fechar"
        >
          <X size={16} />
                ...
                {isOwner ? (
                  <div className={combatStyles.queueControls}>
                    <button
                      type="button"
                      onClick={() =>
                        onMoveQueueEntry(combatRoom.id, entry.id, -1)
                      }
                      disabled={index === 0}
