  ...
  return (
    <div
      className={[
        ...
        isScrollTemplate
          ? parchmentStyles.previewPanelScroll
          : parchmentStyles.previewPanelClassic,
    ...
    >
      ...
      <article className={parchmentStyles.parchmentContent}>
        ...
        {textParagraphs.length > 0 ? (
          ...
          <p className={parchmentStyles.placeholder}>
            A tinta ainda aguarda a ordem.
          </p>
        )}
        {!isScrollTemplate ? (
          <>
            {canPreviewImage(parchment.signatureImage) ? (
              <div className={parchmentStyles.signatureMark}>
                <Image
                  src={parchment.signatureImage.trim()}
                  alt=""
                  width={112}
                  height={112}
                  unoptimized
            ...
            {parchment.signature.trim() ? (
              <p className={parchmentStyles.signature}>
                {parchment.signature.trim()}
              </p>
            ) : null}
          </>
...
export function CampaignParchmentModal({
  ...
  const selectedFontOption = useMemo(
    () =>
      parchmentFontOptions.find((option) => option.value === draft.font) ??
      parchmentFontOptions[0] ??
      null,
    [draft.font]
  )
  const selectedTemplateOption = useMemo(
    () =>
      parchmentTemplateOptions.find(
        (option) => option.value === draft.template
      ) ??
      parchmentTemplateOptions[0] ??
      null,
    [draft.template]
  )
  ...
  async function handleImageChange(
    field: "crestImage" | "signatureImage",
  ...
  return (
    <div className={styles.modalBackdrop} role="presentation" onClick={onClose}>
      ...
      >
            ...
            <ReactSelectField
              ...
              onChange={(option) =>
                updateField(
                  "template",
                  option?.value ??
                    parchmentTemplateOptions[0]?.value ??
                    "classic"
                )
              }
            ...
            <fieldset className={parchmentStyles.imageGroup}>
              ...
              <label className={controlStyles.locationField}>
                ...
                <input
                  ...
                  onChange={(event) =>
                    updateField("crestImage", event.target.value)
                  }
              ...
              <label className={parchmentStyles.fileButton}>
                ...
                <input
                  ...
                  accept="image/*"
                  onChange={(event) =>
                    void handleImageChange("crestImage", event)
                  }
                />
              </label>
            </fieldset>
                ...
                <fieldset className={parchmentStyles.imageGroup}>
                  ...
                  <label className={controlStyles.locationField}>
                    ...
                    <input
                      ...
                      onChange={(event) =>
                        updateField("signatureImage", event.target.value)
                      }
                      placeholder="Opcional"
                    />
                  </label>
                  <label className={parchmentStyles.fileButton}>
                    ...
                    <input
                      ...
                      accept="image/*"
                      onChange={(event) =>
                        void handleImageChange("signatureImage", event)
                      }
                    />
                  </label>
                </fieldset>
          ...
          <div className={parchmentStyles.previewColumn}>
            <ReactSelectField
              ...
              onChange={(option) =>
                updateField(
                  "font",
                  option?.value ?? parchmentFontOptions[0]?.value ?? "cinzel"
                )
              }
        ...
        <svg
          className={parchmentStyles.filterSvg}
          aria-hidden="true"
          focusable="false"
        >
