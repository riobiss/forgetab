...
export async function giveItem(
  ...
  try {
    ...
    if (characterIds.length === 0) {
      throw new AppError(
        "Selecione pelo menos um personagem para receber o item.",
        400,
      )
    }

    const itemExists = await deps.repository.baseItemExists(
      params.rpgId,
      baseItemId,
    )
    if (!itemExists) {
    ...
    const validCharacterIds = new Set(
      await deps.repository.listExistingCharacterIds(
        params.rpgId,
        characterIds,
      ),
    )
    const hasInvalidCharacters = characterIds.some(
      (characterId) => !validCharacterIds.has(characterId),
    )
    if (hasInvalidCharacters) {
      throw new AppError(
        "Um ou mais personagens selecionados nao sao validos neste RPG.",
        400,
      )
    }
