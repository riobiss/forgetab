...
export default function ItemEditorForm({ rpgId, mode, itemId, deps }: Props) {
      ...
      try {
        ...
        setLoadingError(
          cause instanceof Error
            ? cause.message
            : "Erro de conexao ao carregar o item.",
        )
      } finally {
        setLoading(false)
      }
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    if (selectedImageFile) {
      ...
      try {
        const upload = await uploadItemImageUseCase(deps, {
    ...
    const payload: UpsertItemPayloadDto = {
      ...
      durability: toOptionalNumber(durability, (raw) =>
    ...
    try {
      ...
      const message =
        cause instanceof Error
          ? cause.message
          : "Erro de conexao ao salvar o item."
      setSubmitError(message)
      toast.error(message)
    } finally {
        ...
        <form className={styles.form} onSubmit={handleSubmit}>
              ...
              <div key={`ability-${index}`} className={styles.multiCard}>
                <label className={styles.field}>
                  ...
                  <input
                    ...
                    onChange={(event) =>
                      setAbilities((prev) =>
                        updateAt(prev, index, "name", event.target.value),
                      )
                    }
                    placeholder="Opcional"
                  />
          ...
          <div className={styles.multiSection}>
            ...
            <button
              ...
              className={styles.ghostButton}
              onClick={() =>
                setEffects((prev) => [...prev, { name: "", description: "" }])
              }
            >
